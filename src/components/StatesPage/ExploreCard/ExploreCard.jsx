...
const ExploreCard = () => {
  const exploreReview = chunkArray(Explorecarddata, 5);

  return (
    <section className="relative mt-12 max-w-7xl mx-4 xl:mx-auto">
      <header className="flex flex-col gap-2 mb-10">
        <h1 className="text-3xl font-bold text-gray-900 uppercase">Rajasthan Tour Packages by Interest</h1>
      </header>
      <div className="w-full">
        <Carousel theme={customTheme} slideInterval={4000} pauseOnHover>
          {exploreReview.map((group, index) => (
            <div key={index} className="flex gap-5 h-full">
              {group.map((item) => (
                <div key={item.id} className="w-full md:w-1/2 lg:w-1/5 relative">
                  <img
                    src={item.thumbnail}
                    alt={item.tourname}
                  ...
                  <div className="absolute top-0 left-0 w-full h-[200px] flex items-center justify-center bg-transparent rounded-lg transition-colors duration-300 hover:bg-black/50">
                    <h3 className="text-2xl text-white font-bold">{item.tourname}</h3>
                  </div>
                </div>
              ))}
            </div>
